function [binspikes curr_targ_idx other_targ_idx] = bin_spike_4_PEV_cur_and_othr_tg_1stTrlExcd_nonPfct(...   %location last touched 
...
% 試行ごとのTarget location
cellTargLoc = TrialID(2:end,2);

% 試行ごとのother target index (trial-by-trial)
cellOtherTarg = {};
...
for i = 2:trialnum
    if   ActiveTrial(i)  == 1 && ...                 % Trial n had  Reveal event
         ...
         cellCorCount(i) == touchOrder && ...     % what ordinal number is this correct touch in a given cycle?
         cellTouchID{i}  == 1 && ...              % Is this touch target-touch (1) or non-target touch (2)?
         cellTask{i}     == 1 && ....             % task (1->spatial; 2->object)
         cellChoicenum{i} == numBx && ...          % Trial n has numBox number of choice (ie 5 or 6 choice)
         cellTargnum{i}  == targnum && ...        % target condition (1 target / 2 target)
         celltcode{i} ~= excld && ...
         ~isnan(t_trig{i+1, 6}) && ...
         t_trig{i+1, 1} +  Fx_ana_win(1) > 0&& ...
         length(nTrl2OtherTargTch) >= i && ...
         (( cellCyclenum{i} == 1) || (cellCyclenum{i} >= 2 &&  (nTrl2OtherTargTch(i) > 1 || TouchCountInCycle(i) > 1)   )) % nonPerfect trl == cycle 1 は無条件で入れる。
